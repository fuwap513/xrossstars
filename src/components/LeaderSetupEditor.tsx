...
type Props = {
  ...
  onUpdate: (side: Side, leaderId: string, patch: { name?: string; baseHp?: number; sourceCardId?: string }) => void;
  onReset: () => void;
...
export default function LeaderSetupEditor({
  ...
  const officialLeaderCards = useMemo(
    () => cardCatalog
      ...
      .sort((a, b) => {
        const setCompare = (a.officialSet ?? '').localeCompare(b.officialSet ?? '', 'ja-JP');
        if (setCompare !== 0) return setCompare;
        const orderCompare = (a.sortOrder ?? 0) - (b.sortOrder ?? 0);
        if (orderCompare !== 0) return orderCompare;
        return `${a.name}-${a.id}`.localeCompare(`${b.name}-${b.id}`, 'ja-JP');
      }),
    [cardCatalog],
  );
  ...
  const handleNameChange = (side: Side, leaderId: string) => (event: ChangeEvent<HTMLInputElement>) => {
    onUpdate(side, leaderId, { name: event.target.value, sourceCardId: '' });
  };

  const handleHpChange = (side: Side, leaderId: string) => (event: ChangeEvent<HTMLInputElement>) => {
    const rawValue = Number(event.target.value);
    if (Number.isNaN(rawValue)) return;
    onUpdate(side, leaderId, { baseHp: clampHp(rawValue), sourceCardId: '' });
  };

  const handleSelectOfficialLeader = (side: Side, leaderId: string) => (event: ChangeEvent<HTMLSelectElement>) => {
    const selectedCard = officialLeaderCards.find((card) => card.id === event.target.value);
  ...
  const handleImportOfficial = async () => {
    ...
    setImportMessage(success ? '公式カードデータ（第1弾〜第3弾）を読み込みました' : '公式カードデータの読み込みに失敗しました');
  };
  ...
  return (
    <section className="panel leader-setup-panel">
      ...
      <div className="action-row wrap-actions top-gap">
        <button onClick={() => void handleImportOfficial()}>公式1〜3弾を読み込む</button>
        <button className="ghost-button" onClick={onReset}>推奨初期値に戻す</button>
      </div>

      {importMessage ? (
        <div className={`import-status ${importStatus} top-gap`}>{importMessage}</div>
      ) : null}
          ...
          <section className="leader-setup-group compact-card" key={side}>
            ...
            <div className="leader-setup-list top-gap">
              {leaderSetup[side].map((leader) => {
                ...
                const awakenedAtk = parseStat(matchedLeader?.awakenedAtk, baseAtk + 10);
                const awakenedHp = parseStat(matchedLeader?.awakenedHp, baseHp + 30);
                    ...
                    <div className="leader-setup-fields">
                      ...
                      <div className="detail-box">
                        <div className="detail-chip-row">
                          <span className="detail-chip">{matchedLeader?.name ?? leader.name}</span>
                          {matchedLeader?.officialCardNumber ? (
                            <span className="detail-chip detail-chip-muted">{matchedLeader.officialCardNumber}</span>
                          ) : null}
                          {matchedLeader?.color ? (
                            <span className="detail-chip detail-chip-success">{matchedLeader.color}</span>
                          ) : null}
                        </div>
                        <div className="detail-description">
                          通常 ATK {baseAtk} / HP {baseHp} ・ 覚醒 ATK {awakenedAtk} / HP {awakenedHp}
                        </div>
                      </div>
